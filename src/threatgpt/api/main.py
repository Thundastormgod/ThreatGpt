"""FastAPI application for ThreatGPT API.
...
from threatgpt.core.models import ThreatScenario, SimulationResult, ThreatType
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting ThreatGPT API...")
    try:
        llm_manager = LLMManager()
        simulator = ThreatSimulator(llm_provider=llm_manager)
        logger.info("ThreatGPT API started successfully")
    except Exception as e:
        logger.error(f"Failed to initialize ThreatGPT API: {str(e)}")
        raise
    
    yield
    
    # Shutdown
    logger.info("Shutting down ThreatGPT API...")
